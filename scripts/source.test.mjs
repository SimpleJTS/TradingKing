...
test('content script keeps OpenFour-ready floating shell and dual-chain/debot detection', () => {
  ...
  assert.match(contentSource, /data-buy-amount/);
  assert.match(contentSource, /data-sell-percent/);
  assert.match(contentSource, /SETTINGS_STORAGE_KEY/);
  assert.match(contentSource, /slippageBps/);
  assert.match(contentSource, /gasFeeGwei/);
  assert.match(contentSource, /data-import-wallet/);
